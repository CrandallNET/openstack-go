...
Implemented live Identity commands currently include `token issue`, `catalog list/show`, `domain list/show`, `endpoint list/show`, `group list/show`, `project list/show`, `region list/show`, `role list/show`, `service list/show`, and `user list/show`. The first non-Identity read commands include `server list/show`, `flavor list/show`, `image list/show`, `network list/show`, `volume list/show`, `subnet list/show`, `port list/show`, `floating ip list/show`, `keypair list/show`, `server group list`, `volume type list/show`, and `volume snapshot list`. `server group show` and `volume snapshot show` are implemented, but still need live fixtures. The table renderer accepts `--max-width`, `--fit-width`, `CLIFF_MAX_TERM_WIDTH`, and `CLIFF_FIT_WIDTH=1`. The enhanced human output remains opt-in through `--format=pretty` or `--pretty`.
