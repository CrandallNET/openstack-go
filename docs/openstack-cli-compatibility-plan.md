...
| Implement Gophercloud client factories | Auth/config resolution | `in-progress` | Shared provider setup and an Identity v3 service client factory exist. `token issue` extracts Identity v3 token data, and the first Identity read commands use the shared service client. Other service factories are not implemented yet. | Add endpoint selection, interface/region handling, microversions, debug logging, timing, and service-client creation for Compute, Network, Volume, Image, Object Store, and Placement. |
