...
| Implement parser compatibility layer | Cobra/pflag registry | `in-progress` | Initial Cobra/pflag root command and global flag skeleton exist; compatibility help, errors, completion, sorting, and generated command metadata still need oracle-backed work. | Override help, usage, errors, command sorting, suggestions, option placement behavior, and `openstack complete`. |
...
| Implement CLI-local commands | Parser layer; renderers | `in-progress` | `--version`, root `--help`, `help`, `command list`, `complete`, `module list`, and `configuration show` are wired. Stub-only commands still print the required not-implemented message until catalog data and renderers exist. | Replace stubs with catalog-backed behavior and renderer-backed output. |
