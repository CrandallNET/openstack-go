...
The current implementation uses Gophercloud's Block Storage [attachments](https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/blockstorage/v3/attachments) and [QoS](https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/blockstorage/v3/qos) packages for `volume attachment list/show` and `volume qos list/show`. `volume summary`, `block storage resource filter list/show`, and `block storage log level list` use narrow Cinder REST reads because no typed Gophercloud helper exists in the local v2.12.0 module for those commands. The OpenStack Block Storage API documents `GET /v3/{project_id}/volumes/summary` as available since microversion 3.12, with `metadata` added in microversion 3.36; the pinned local OpenStackSDK source maps resource filters to `/resource_filters` and log-level reads to `/os-services/get-log`.
