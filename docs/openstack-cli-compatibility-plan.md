...
The current local command surface includes cached image operations, stores, and extensive metadef namespace/object/property/resource-type commands. Those packages were not visible in the cited Gophercloud v2 package index, so they are initial SDK gaps until module inspection proves otherwise. `cached image list`, `image metadef namespace list/show`, `image metadef resource type list`, and `image metadef resource type association list` now use narrow Glance REST reads because no typed local Gophercloud helpers exist in v2.12.0. Plan for raw REST shims for the remaining cache commands, metadef object/property commands, stores, and any import workflow details not covered by `imageimport`.
