...
Gaps to validate are default security group rules, local IPs and local IP associations, network flavor and flavor profile commands, L3 conntrack helpers, network meters and meter rules, segment ranges, exact QoS rule subtypes, trunk subport edge cases, and Tap-as-a-Service tap flow and tap service commands. Tap mirror support is visible in Gophercloud, but tap flow and tap service support were not visible in the cited package search, so treat those as gap candidates. Floating IP port forwarding still needs a live cloud fixture because `cloud6` returns Neutron `404` for the `floatingips/{id}/port_forwardings` endpoint in both Python OSC and the Go CLI.
...
The current implementation uses Gophercloud's Block Storage [attachments](https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/blockstorage/v3/attachments) and [QoS](https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/blockstorage/v3/qos) packages for `volume attachment list/show` and `volume qos list/show`. `volume summary`, `block storage cluster list/show`, `block storage resource filter list/show`, `block storage log level list`, `volume group list/show`, `volume group snapshot list/show`, `volume group type list/show`, and `volume message list/show` use narrow Cinder REST reads because no typed Gophercloud helper exists in the local v2.12.0 module for those commands. The OpenStack Block Storage API documents `GET /v3/{project_id}/volumes/summary` as available since microversion 3.12, with `metadata` added in microversion 3.36; the pinned local OpenStackSDK source maps resource filters to `/resource_filters`, log-level reads to `/os-services/get-log`, groups to `/groups`, group snapshots to `/group_snapshots`, and group types to `/group_types`, while the pinned local cinderclient source maps clusters to `/clusters` and messages to `/messages`.

Gaps to validate are consistency groups, consistency group snapshots, group failover, log level set behavior, cleanup, manageable snapshots, backend capability display, volume revert, backup record export/import, host set semantics, encryption subcommands on volume types, and exact scheduler stats output. These should be raw REST shims unless Gophercloud packages exist after direct module inspection.

### Common and Cross-Service Commands

`availability zone list`, `extension list/show`, `limits show`, `quota *`, `versions show`, and `project cleanup` are not single-resource wrappers. They aggregate multiple services, depend on active API versions, and often have service-specific option behavior. Implement these as first-class compatibility commands using Gophercloud clients underneath, not as thin SDK package calls. `quota set/delete` now have initial Compute, Volume, and Network implementations through authenticated service clients and narrow quota REST requests, including Compute and Volume quota-class updates. They still need dedicated project fixtures before live mutation tests can run safely.
...
| Seed command compatibility matrix | OSC command catalog | `done` | `tools/compat-matrix` generated `compat/matrix.yaml` with one row per local command, initial status, service/API mapping, plugin-scope markers, and the implemented `command list` row. | Fill SDK package coverage, shim requirements, command-specific tests, and status changes as service work lands. |
| Seed cloud test matrix | Cloud test matrix in this plan | `done` | `tools/compat-matrix` generated `compat/test-matrix.yaml` and non-secret `compat/test-clouds.yaml` from the agreed cloud safety rules. | Keep the YAML files updated as live discovery adds service and fixture details. |
...
| Implement parser compatibility layer | Cobra/pflag registry | `in-progress` | Initial root/global parsing exists, including table formatter width flags, auth flags, common formatter flags, and command-local flags for the first Identity read commands. Cataloged command stubs ignore unknown command flags, leaf help uses captured OSC help snapshots, and `complete` emits the captured OSC completion script. Error formatting, root help parity, complete command-local flag coverage, sorting, aliases, and option placement still need oracle-backed work. | Override remaining help, usage, errors, command sorting, suggestions, option placement behavior, and command-local flags. |
...
| Roll out service read commands | Identity bootstrap; lookup/error helpers | `in-progress` | Compute, Image, Network, Volume, Object Store, Placement, and Common now have initial read slices. Cloud-verified commands include the earlier core list/show commands plus `access rule list`, `application credential list`, `credential list`, `ec2 credentials list`, `identity provider list`, `implied role list`, `limit list`, `mapping list`, `policy list`, `registered limit list`, `role assignment list`, `service provider list`, `trust list`, `aggregate list`, `compute agent list`, `compute service list`, `console connection show`, `console log show`, `console url show`, `host list/show`, `hypervisor list/show`, `hypervisor stats show`, `server event list/show`, `server migration list`, `server volume list`, `usage list/show`, `cached image clear/delete/list/queue`, `image create/delete/import/save/set/stage/unset`, `image import info`, `image member list`, `image metadef namespace create/delete/list/set/show`, `image metadef object create/delete/list/show/update`, `image metadef object property show`, `image metadef property create/delete/list/set/show`, `image metadef resource type list`, `image metadef resource type association create/delete/list`, `image stores list`, `image task list`, `address group list`, `address scope list`, `floating ip pool list`, `ip availability list/show`, `network agent list/show`, `network rbac list/show`, `network service provider list`, `router list/show`, `security group list/show`, `security group rule list/show`, `subnet pool list`, `block storage cluster list`, `block storage log level list`, `block storage resource filter list/show`, `volume attachment list/show`, `volume group list`, `volume group snapshot list`, `volume group type list`, `volume message list/show`, `volume qos list/show`, `volume summary`, `volume backup list`, `volume service list`, `volume backend pool list`, `volume transfer request list`, `container list/show`, `object list/show`, `object store account show`, `allocation candidate list`, `resource class list/show`, `resource provider list/show`, resource provider aggregate/allocation/inventory/trait/usage reads, `trait list/show`, `availability zone list`, `extension list/show`, `limits show`, `quota list/show`, and `versions show`. `access rule show`, `application credential show`, `credential show`, `ec2 credentials show`, `federation protocol list/show`, `identity provider show`, `limit show`, `mapping show`, `policy show`, `registered limit show`, `service provider show`, `trust show`, `address group show`, `address scope show`, `aggregate show`, `block storage cluster show`, `image member get`, `image task show`, `network qos policy list/show`, `network qos rule type list/show`, `network segment list/show`, `network trunk list/show`, `server group show`, `subnet pool show`, `volume backup show`, `volume group show`, `volume group snapshot show`, `volume group type show`, `volume snapshot show`, and `volume transfer request show` are implemented but still need live fixtures or a cloud exposing the relevant extension. Lookup/error helpers are still partial, so exact OSC parity is not complete. | Add missing fixtures and run remote service breadth smoke for Network extensions absent from `cloud6`, then add oracle golden tests before expanding write coverage. |
...
| Implement service write commands | Write lifecycle framework | `in-progress` | Low-risk Image writes now include `image create`, `image delete`, `image set`, `image unset`, `image stage`, `image import`, `image add project`, `image remove project`, Glance cache mutations, and Glance metadef namespace/object/property/resource-type lifecycle commands. Compute writes now include `keypair create/delete` and `server group create/delete` with disposable lifecycle verification on `cloud6`. Network writes now include `address group create/delete/set/unset`, `address scope create/delete/set`, `floating ip create/delete/set/unset`, `floating ip port forwarding create/delete/list/set/show`, `network create/delete/set/unset`, `network qos policy create/delete/set`, `network rbac create/delete/set`, `port create/delete/set/unset`, `router add/remove gateway`, `router add/remove port`, `router add/remove route`, `router add/remove subnet`, `router create/delete/set/unset`, `security group create/delete/set/unset`, `security group rule create/delete`, `subnet create/delete/set/unset`, and `subnet pool create/delete/set/unset`, including project lookup, stateful/stateless selection, standard tags, address-scope association, floating IP allocation, port lifecycle, router interface attachment, router extra-route actions, QoS policy false-boolean request handling, RBAC target-project handling, port-forwarding range parsing, and Python-compatible raw show output. The non-port-forwarding and non-QoS Neutron lifecycle commands are verified with disposable floating IPs, groups, networks, network RBAC policies, ports, routers, router interfaces, routes, scopes, subnets, subnet pools, and rules on `cloud6`; port forwarding and QoS policy commands are implemented but still need a cloud exposing those endpoints because `cloud6` returns the same Neutron `404` for Python OSC and the Go CLI. Router gateway add/remove are implemented and match Python's `cloud6` extension-gated error, but successful gateway action verification needs a cloud exposing `external-gateway-multihoming`. Object Store writes now include `container create/delete/set/unset` and `object create/delete/save/set/unset`, verified with disposable containers and objects on `flex-sjc` because `cloud6` currently has no usable object-store public endpoint. Common admin writes now include initial `quota set` and `quota delete` support for Compute, Volume, and Network quotas, with Compute and Volume quota-class updates for `--class` and `--default`. `image create`, `image delete`, `image save`, `image set`, `image unset`, `image stage`, and `image import` passed disposable-image lifecycle checks on `cloud6`, including queued metadata create, direct zero-byte file upload, save, property/tag mutation, stage/import, cleanup, and the Python-compatible nonexistent-image delete error. Quota mutations are implemented but not live-verified because they require a dedicated test project and before/after quota capture. | Add automated setup/teardown fixtures, then continue with remaining command-level parity and less common image set modes such as current-project membership without `--project`. |
| Close Gophercloud SDK gaps with shims | Command matrix; service implementations | `in-progress` | Narrow shims now cover Neutron availability zones, OSC-shaped quota aggregation, quota reset/update requests, service-catalog version discovery, Identity federation/provider REST reads, Neutron service-provider reads, Nova console auth token lookup, Nova deprecated `os-agents` and `os-hosts` reads, Nova migration list reads, Glance cache operations, Glance image activation, Glance image import, Glance image multi-store delete, Glance image tag delete, Glance metadef namespace/object/property/resource-type read and write operations, Cinder volume summary, Cinder clusters, Cinder groups, Cinder group snapshots, Cinder group types, Cinder resource filters, Cinder log-level reads, and Cinder messages. They are implemented through authenticated Gophercloud clients where typed packages are missing or do not preserve the Python output shape. The matrix marks these package gaps explicitly. | Move shims behind service-scoped extras packages/plugins as that plugin boundary is built, and add mocked HTTP tests for each shim endpoint before expanding beyond read-only calls. |
