...
The current local command surface includes cached image operations, stores, and extensive metadef namespace/object/property/resource-type commands. Those packages were not visible in the cited Gophercloud v2 package index, so they are initial SDK gaps until module inspection proves otherwise. `cached image list` now uses a narrow Glance `/cache` REST read, and `image metadef namespace list/show` use narrow Glance `/metadefs/namespaces` REST reads because no typed local Gophercloud helpers exist in v2.12.0. Plan for raw REST shims for the remaining cache commands, metadef object/property/resource-type commands, stores, and any import workflow details not covered by `imageimport`.
