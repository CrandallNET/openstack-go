...
| Implement parser compatibility layer | Cobra/pflag registry | `in-progress` | Initial root/global parsing exists. Cataloged command stubs ignore unknown command flags, leaf help uses captured OSC help snapshots, and `complete` emits the captured OSC completion script. Error formatting, root help parity, command-local flag parsing, sorting, aliases, and option placement still need oracle-backed work. | Override remaining help, usage, errors, command sorting, suggestions, option placement behavior, and command-local flags. |
