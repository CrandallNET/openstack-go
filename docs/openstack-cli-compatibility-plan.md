...
| Close Gophercloud SDK gaps with shims | Command matrix; service implementations | `in-progress` | The first narrow shims are Neutron availability zones and OSC-shaped quota aggregation, implemented through authenticated Gophercloud service clients where typed packages are missing or do not preserve the Python output shape. The matrix marks these package gaps explicitly. | Move shims behind service-scoped extras packages/plugins as that plugin boundary is built, and add mocked HTTP tests for each shim endpoint before expanding beyond read-only calls. |
