...
| Roll out service read commands | Identity bootstrap; lookup/error helpers | `in-progress` | `server list/show`, `flavor list/show`, `image list/show`, `network list/show`, and `volume list/show` are implemented through Gophercloud and passed live JSON smoke checks on `cloud6`. Lookup/error helpers are still partial, so exact OSC parity is not complete. | Continue with subnet, port, floating IP, keypair, server group, volume type, snapshot, object store, and Placement list/show commands, then add oracle golden tests. |
