...
Remaining gaps are no longer command stubs; they are validation gaps. Volume v3 still needs oracle-backed default-output tests, mocked endpoint tests for each raw Cinder shim, and safe lifecycle suites for disposable volumes, snapshots, backups, transfers, QoS specs, volume types, group types, and group snapshots. Admin-only commands such as cleanup, host set, log-level set, message delete, migration, revert, and group failover need `cloud6` policy checks and retained diagnostics before promotion beyond `implemented`.
