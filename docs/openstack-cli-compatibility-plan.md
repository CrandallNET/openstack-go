...
| Roll out service read commands | Identity bootstrap; lookup/error helpers | `in-progress` | `server list/show`, `flavor list/show`, `image list/show`, `network list/show`, `volume list/show`, `subnet list/show`, `port list/show`, `floating ip list/show`, `keypair list/show`, `server group list`, `volume type list/show`, and `volume snapshot list` are implemented through Gophercloud and passed live JSON smoke checks on `cloud6`. `server group show` and `volume snapshot show` are implemented but still need live fixtures. Lookup/error helpers are still partial, so exact OSC parity is not complete. | Continue with object store and Placement list/show commands, add missing fixtures for `server group show` and `volume snapshot show`, then add oracle golden tests. |
