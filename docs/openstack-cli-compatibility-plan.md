...
| Implement Gophercloud client factories | Auth/config resolution | `in-progress` | Shared provider setup and service-client helpers exist for Identity v3, Compute v2, Image v2, Network v2, Block Storage v3, Object Storage v1, and Placement v1. `token issue`, the implemented Identity read commands, and the first cross-service read commands use these helpers. Placement sets an explicit default microversion when `OS_PLACEMENT_API_VERSION` is absent. | Add endpoint selection parity, interface/region edge cases, debug logging, timing, complete microversion flag parsing, and remaining service-specific client setup details. |
...
| Roll out service read commands | Identity bootstrap; lookup/error helpers | `in-progress` | Compute, Image, Network, Volume, Object Store, Placement, and Common now have initial read slices. Cloud-verified commands include the earlier core list/show commands plus `container list/show`, `object list/show`, `object store account show`, `allocation candidate list`, `resource class list/show`, `resource provider list/show`, resource provider aggregate/allocation/inventory/trait/usage reads, `trait list/show`, `availability zone list`, `extension list/show`, and `limits show`. `server group show` and `volume snapshot show` are implemented but still need live fixtures. Lookup/error helpers are still partial, so exact OSC parity is not complete. | Add missing fixtures for `server group show` and `volume snapshot show`, continue with `quota list/show` and `versions show`, then add oracle golden tests. |
...
| Close Gophercloud SDK gaps with shims | Command matrix; service implementations | `in-progress` | The first narrow shim is `availability zone list` for Neutron availability zones, implemented through the authenticated Gophercloud Network v2 service client because no dedicated Gophercloud package was found in v2.12.0. The matrix marks this package gap explicitly. | Move shims behind service-scoped extras packages/plugins as that plugin boundary is built, and add mocked HTTP tests for each shim endpoint before expanding beyond read-only calls. |
