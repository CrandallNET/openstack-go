...
| Implement Gophercloud client factories | Auth/config resolution | `in-progress` | Shared provider setup and service-client helpers exist for Identity v3, Compute v2, Image v2, Network v2, and Block Storage v3. `token issue`, the first Identity read commands, and the first cross-service read commands use these helpers. Object Store and Placement helpers are not implemented yet. | Add endpoint selection, interface/region handling, microversions, debug logging, timing, Object Store, Placement, and service-specific client setup details. |
...
| Roll out service read commands | Identity bootstrap; lookup/error helpers | `in-progress` | Compute, Image, Network, Volume, Object Store, and Placement now have initial Gophercloud-backed read slices. Cloud-verified commands include the earlier core list/show commands plus `container list/show`, `object list/show`, `object store account show`, `allocation candidate list`, `resource class list/show`, `resource provider list/show`, resource provider aggregate/allocation/inventory/trait/usage reads, and `trait list/show`. `server group show` and `volume snapshot show` are implemented but still need live fixtures. Lookup/error helpers are still partial, so exact OSC parity is not complete. | Continue with common read commands, add missing fixtures for `server group show` and `volume snapshot show`, then add oracle golden tests. |
