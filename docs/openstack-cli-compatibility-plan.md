...
Gophercloud has documented Image v2 packages for image data, image import, images, members, and tasks. That should cover core `image create/delete/list/save/set/show/stage/import`, import discovery, image members, and tasks. `image create` uses typed image metadata creation plus typed direct upload/stage/import calls for file data, and `image delete` uses typed image deletion for normal deletes. `image delete --store` uses a narrow authenticated Glance REST delete because the local Gophercloud v2.12.0 module has no typed multi-store delete helper. `image import info` uses Gophercloud's typed `imageimport` package, and `image add project`/`image remove project` use the typed image member package.
...
| Implement service write commands | Write lifecycle framework | `in-progress` | Low-risk Image writes now include `image create`, `image delete`, `image add project`, `image remove project`, Glance cache mutations, and Glance metadef namespace/object/property/resource-type lifecycle commands. `image create` and `image delete` passed disposable-image lifecycle checks on `cloud6`, including queued metadata create, direct zero-byte file upload, cleanup, and the Python-compatible nonexistent-image delete error. | Add automated setup/teardown fixtures, then continue with `image set`, `image save`, `image stage`, and `image import` command-level parity. |
| Close Gophercloud SDK gaps with shims | Command matrix; service implementations | `in-progress` | Narrow shims now cover Neutron availability zones, OSC-shaped quota aggregation, service-catalog version discovery, Identity federation/provider REST reads, Neutron service-provider reads, Nova console auth token lookup, Nova deprecated `os-agents` and `os-hosts` reads, Nova migration list reads, Glance cache operations, Glance image multi-store delete, Glance metadef namespace/object/property/resource-type read and write operations, Cinder volume summary, Cinder clusters, Cinder groups, Cinder group snapshots, Cinder group types, Cinder resource filters, Cinder log-level reads, and Cinder messages. They are implemented through authenticated Gophercloud clients where typed packages are missing or do not preserve the Python output shape. The matrix marks these package gaps explicitly. | Move shims behind service-scoped extras packages/plugins as that plugin boundary is built, and add mocked HTTP tests for each shim endpoint before expanding beyond read-only calls. |
