...
Gaps to validate are default security group rules, local IPs and local IP associations, network flavor and flavor profile commands, L3 conntrack helpers, network meters and meter rules, segment ranges, trunk subport edge cases, and Tap-as-a-Service tap flow and tap service commands. Tap mirror support is visible in Gophercloud, but tap flow and tap service support were not visible in the cited package search, so treat those as gap candidates. Floating IP port forwarding still needs a live cloud fixture because `cloud6` returns Neutron `404` for the `floatingips/{id}/port_forwardings` endpoint in both Python OSC and the Go CLI. QoS rule commands are implemented, but `minimum-packet-rate` remains a documented Gophercloud package gap in v2.12.0 and is handled by a narrow authenticated Neutron request.
