...
| Implement parser compatibility layer | Cobra/pflag registry | `in-progress` | Initial root/global parsing exists, including table formatter width flags and auth flags. Cataloged command stubs ignore unknown command flags, leaf help uses captured OSC help snapshots, and `complete` emits the captured OSC completion script. Error formatting, root help parity, command-local flag parsing, sorting, aliases, and option placement still need oracle-backed work. | Override remaining help, usage, errors, command sorting, suggestions, option placement behavior, and command-local flags. |
...
| Implement OSC-compatible renderers | Go module skeleton | `in-progress` | Initial table renderer supports Cliff-style `--max-width`, `--fit-width`, `CLIFF_MAX_TERM_WIDTH`, and `CLIFF_FIT_WIDTH` behavior. JSON and value output exist for the first local and token commands, and `pretty` is wired for `token issue`. | Add full `table`, `json`, `yaml`, `csv`, `value`, `shell`, and enhanced `pretty` renderers with oracle-backed golden tests, including `--column`, `--quote`, `--noindent`, `--prefix`, sorting, and empty-output behavior. |
...
| Implement Gophercloud client factories | Auth/config resolution | `in-progress` | `token issue` creates a Gophercloud provider client and extracts Identity v3 token data. Per-service factories are not implemented yet. | Add endpoint selection, interface/region handling, microversions, debug logging, timing, and service-client creation for each service. |
...
| Implement Identity read bootstrap | Auth/config; client factories; renderers | `in-progress` | `token issue` is implemented through Gophercloud, has an injected unit test, is marked `cloud-verified` in `compat/matrix.yaml`, and passed on `cloud6`. | Add oracle parity tests for `token issue`, then implement catalog, project, domain, user, group, role, service, and endpoint read commands. |
