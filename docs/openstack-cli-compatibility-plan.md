...
Known or likely gaps to validate are `compute agent create/delete/set`, `host set`, `aggregate cache image`, `server ssh` local SSH orchestration, and the exact admin/microversion behavior of evacuation, rescue, resize, shelve, unshelve, lock, unlock, migration, and volume attachment update commands. Server write commands now need disposable live tests and Python OSC output comparisons before they can be promoted beyond implementation coverage. Any missing SDK method should be implemented as a narrow raw REST shim first, with an upstream Gophercloud contribution considered after compatibility tests prove the behavior.
