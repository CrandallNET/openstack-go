...
| Roll out service read commands | Identity bootstrap; lookup/error helpers | `in-progress` | Compute, Image, Network, Volume, Object Store, Placement, and Common now have initial read slices. `tools/compat-check` supports live fixture placeholders such as `{server}`, `{volume}`, and `{network}` so show commands can use IDs discovered from the Python oracle at run time. Static and live oracle parity has promoted `command list` plus 23 `cloud6` read commands to `golden-matched`: `flavor list/show`, `hypervisor list`, `image list/show`, `keypair list/show`, `network list/show`, `port list/show`, `project list/show`, `router list/show`, `security group list/show`, `server list/show`, `subnet list/show`, and `volume list/show`. The `cloud6` live suite now passes for both default table output and JSON output for those commands, with only the documented static known gaps for root help ordering and Go module reporting. | Keep promoting the remaining implemented and cloud-verified commands through fixture-aware live comparisons, including command flags, alternate formats, write lifecycle output, admin-only commands, and remote service breadth smoke on the flex clouds. |
