...
| Add parser and help parity tests | Parser compatibility layer | `in-progress` | `tools/compat-check` compares the pinned Python OSC oracle with `./bin/openstack` for static no-cloud cases covering completion, command-list output, representative leaf help, and known parser gaps. `make compat-static` runs the required subset, and `make compat-static-all` adds `--help` parity for every cataloged command. | Expand required cases to root help, invalid command errors, invalid flag errors, and more default-output commands as those compatibility gaps are closed. |
| Implement OSC-compatible renderers | Go module skeleton | `in-progress` | Initial table renderer supports Cliff-style `--max-width`, `--fit-width`, `CLIFF_MAX_TERM_WIDTH`, and `CLIFF_FIT_WIDTH` behavior. Shared list/show renderers now cover `table`, `json`, `yaml`, `csv`, `value`, `shell` for show output, column selection, noindent, prefixes, and basic sorting. JSON list/show output preserves the command's declared column or field order instead of Go map ordering. The Go-only `pretty` path now uses Charm Bubbles table output and has a reusable Bubbles progress helper for future wait loops without changing default OSC output. | Add oracle-backed golden tests and close remaining formatter gaps, including exact CSV quote modes, shell escaping, empty-output behavior, command-specific default columns, and command-level wait progress integration. |
...
| Add live cloud discovery | Auth/config resolution | `in-progress` | `tools/cloud-discovery` and `make discover-cloud CLOUD=name[,name]` write non-secret JSON reports under `compat/live-clouds/`, including service catalog endpoints and safe fixture candidates for images, flavors, networks, and volume types. Role discovery and structured test eligibility are still open. | Run discovery for `cloud6`, `flex-sjc`, `flex-dfw`, and `flex-iad`; then add role discovery, API version details, extension details, selected fixture diagnostics, and structured skip reasons. |
