...
| Implement CLI-local commands | Parser layer; renderers | `in-progress` | `--version`, root `--help`, `help`, `complete`, and catalog-backed `command list` are wired. `module list` and `configuration show` remain generated stubs that print the required not-implemented message. | Replace remaining stubs with compatibility behavior and renderer-backed output. |
