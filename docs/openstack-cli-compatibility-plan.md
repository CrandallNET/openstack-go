...
Latest write-output parity evidence, 2026-05-06: `make server-lifecycle CLOUD=cloud6`, `make volume-lifecycle CLOUD=cloud6`, and `make quota-lifecycle CLOUD=cloud6` passed after adding paired Python-vs-Go output comparisons to the disposable lifecycle runner. The server suite now compares safe server lifecycle write output for create/delete, set/unset, lock/unlock, reboot, stop/start, rebuild, pause/unpause, suspend/resume, rescue/unrescue, volume attach/detach/set/update, security-group add/remove, port add/remove, and network add/remove. The Volume suite compares `volume attachment create/set/complete/delete`, and the quota suite compares project-scoped `quota set` and `quota delete` output while preserving the existing `quota show -f json` oracle checks before and after reset.
