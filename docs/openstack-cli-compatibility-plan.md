...
| Roll out service read commands | Identity bootstrap; lookup/error helpers | `in-progress` | Compute, Image, Network, Volume, Object Store, Placement, and Common now have initial read slices. Cloud-verified commands include the earlier core list/show commands plus `aggregate list`, `compute service list`, `hypervisor list/show`, `hypervisor stats show`, `image member list`, `image stores list`, `image task list`, `router list/show`, `security group list/show`, `security group rule list/show`, `container list/show`, `object list/show`, `object store account show`, `allocation candidate list`, `resource class list/show`, `resource provider list/show`, resource provider aggregate/allocation/inventory/trait/usage reads, `trait list/show`, `availability zone list`, `extension list/show`, `limits show`, `quota list/show`, and `versions show`. `aggregate show`, `image member get`, `image task show`, `server group show`, and `volume snapshot show` are implemented but still need live fixtures. Lookup/error helpers are still partial, so exact OSC parity is not complete. | Add missing fixtures for `aggregate show`, `image member get`, `image task show`, `server group show`, and `volume snapshot show`, then add oracle golden tests before expanding write coverage. |
