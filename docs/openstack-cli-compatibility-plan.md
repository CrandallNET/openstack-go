...
Gophercloud has documented Identity v3 packages for application credentials and access rules, catalog, credentials, domains, EC2 credentials and tokens, endpoints, federation, groups, limits, OAuth1, inherited roles, policies, project endpoints, projects, regions, registered limits, roles, services, tokens, trusts, and users. That is strong coverage for the local 128-command Identity surface. The current implementation also uses the roles package for `role assignment list`, with OSC-shaped extraction for system and inherited scope fields that the local typed Gophercloud assignment struct does not expose.

Gaps to validate are endpoint group commands, service provider commands, implied role commands, request/access token command details, remaining role-assignment edge cases, federation protocol/provider edge cases, application credential access-rule option parity, and OSC's domain-scoped name resolution. Authentication plugin parity is tracked separately because it affects every service.
...
The current implementation uses Gophercloud's Block Storage [attachments](https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/blockstorage/v3/attachments) and [QoS](https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/blockstorage/v3/qos) packages for `volume attachment list/show` and `volume qos list/show`. `volume summary` uses a narrow Cinder REST read because no typed Gophercloud helper exists in the local v2.12.0 module; the OpenStack Block Storage API documents `GET /v3/{project_id}/volumes/summary` as available since microversion 3.12, with `metadata` added in microversion 3.36.
...
| Implement Identity read bootstrap | Auth/config; client factories; renderers | `in-progress` | `token issue`, `catalog list/show`, `domain list/show`, `endpoint list/show`, `group list/show`, `project list/show`, `region list/show`, `role list/show`, `role assignment list`, `service list/show`, and `user list/show` are implemented through Gophercloud. Available fixtures passed live JSON smoke checks on `cloud6`, and `compat/matrix.yaml` records current command status. | Add oracle parity tests, complete remaining filters and domain-aware lookup semantics, then continue with Identity write/admin commands. |
| Roll out service read commands | Identity bootstrap; lookup/error helpers | `in-progress` | Compute, Image, Network, Volume, Object Store, Placement, and Common now have initial read slices. Cloud-verified commands include the earlier core list/show commands plus `access rule list`, `application credential list`, `credential list`, `ec2 credentials list`, `limit list`, `policy list`, `registered limit list`, `role assignment list`, `trust list`, `aggregate list`, `compute service list`, `console log show`, `console url show`, `hypervisor list/show`, `hypervisor stats show`, `server event list/show`, `server volume list`, `usage list/show`, `image member list`, `image stores list`, `image task list`, `address group list`, `address scope list`, `network agent list/show`, `network rbac list/show`, `router list/show`, `security group list/show`, `security group rule list/show`, `subnet pool list`, `volume attachment list/show`, `volume qos list/show`, `volume summary`, `volume backup list`, `volume service list`, `volume backend pool list`, `volume transfer request list`, `container list/show`, `object list/show`, `object store account show`, `allocation candidate list`, `resource class list/show`, `resource provider list/show`, resource provider aggregate/allocation/inventory/trait/usage reads, `trait list/show`, `availability zone list`, `extension list/show`, `limits show`, `quota list/show`, and `versions show`. `access rule show`, `application credential show`, `credential show`, `ec2 credentials show`, `limit show`, `policy show`, `registered limit show`, `trust show`, `address group show`, `address scope show`, `aggregate show`, `image member get`, `image task show`, `network qos policy list/show`, `network qos rule type list/show`, `network segment list/show`, `network trunk list/show`, `server group show`, `subnet pool show`, `volume backup show`, `volume snapshot show`, and `volume transfer request show` are implemented but still need live fixtures or a cloud exposing the relevant extension. Lookup/error helpers are still partial, so exact OSC parity is not complete. | Add missing fixtures and run remote service breadth smoke for Network extensions absent from `cloud6`, then add oracle golden tests before expanding write coverage. |
...
| Close Gophercloud SDK gaps with shims | Command matrix; service implementations | `in-progress` | The first narrow shims are Neutron availability zones, OSC-shaped quota aggregation, and service-catalog version discovery, implemented through authenticated Gophercloud clients where typed packages are missing or do not preserve the Python output shape. The matrix marks these package gaps explicitly. | Move shims behind service-scoped extras packages/plugins as that plugin boundary is built, and add mocked HTTP tests for each shim endpoint before expanding beyond read-only calls. |
