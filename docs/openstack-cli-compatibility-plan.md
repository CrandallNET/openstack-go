...
| A-031 | `decided` | Pretty OS image colors shown by `make os-test` should preserve brand recognition but must pass at least 4.5:1 contrast against the dark Fancy terminal baseline. When a primary logo color is hard to read, use a lighter color from the same documented palette or a reviewed accessible tint, and report measured contrast in `make os-test`. | User requested readable OS colors; WCAG 2.2 SC 1.4.3 uses 4.5:1 for normal text contrast. |
