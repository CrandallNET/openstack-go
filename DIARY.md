...
## 2026-05-08: Neutron Extras Plugin Coverage

Decision: implement the remaining `openstack.network.v2` command stubs through a new `openstack.commands.extras.neutron-extras` module. These commands are Neutron extension surfaces where the existing core implementation either had no typed local Gophercloud package coverage or needed Python OSC-shaped raw output and request bodies. Keeping them in `neutron-extras` follows the earlier service-scoped plugin decision and keeps raw REST compatibility shims separate from core typed Gophercloud commands.

Work done: added the `neutron-extras` Caddy module and registered the remaining Network v2 stubs as implemented: default security group rules, local IPs and local IP associations, network agent action/set/delete commands, auto-allocated topology, network flavors and service profiles, L3 conntrack helpers, metering labels and rules, network segment ranges, router NDP proxies, and Tap-as-a-Service tap service, tap flow, and tap mirror commands. The CLI handler uses the authenticated Gophercloud Network v2 service client and command-specific Neutron REST resource specs for list, show, create, update, and delete behavior. It resolves projects, networks, ports, and routers through existing helpers where Python OSC accepts names, and leaves the command output on the shared renderer path so default output remains the Python-compatible mode and Pretty remains Go-only.

Compatibility fixes found during validation: `default security group rule list` initially used raw Neutron field names and returned `None` for empty port ranges. The Python oracle uses the headers `IP Protocol`, `IP Range`, `Remote Security Group`, `Used in default Security Group`, and `Used in custom Security Group`, renders an empty port range as an empty string, and synthesizes `0.0.0.0/0` or `::/0` when Neutron returns a null remote IP prefix. The Go list renderer now mirrors that observed output.

Validation: added unit coverage for `neutron-extras` module registration, command-list implementation state, default security group rule list request/output shaping, and network flavor create request shaping. `go test ./internal/cli ./internal/plugins/neutronextras ./tools/matrix`, `go test ./...`, and `make build` passed. `tools/matrix` now reports zero `unknown` rows for `openstack.network.v2`; the global unknown count is 117 outside Network v2. Live `cloud6` Python-vs-Go parity passed for the safe read-only list commands in both JSON and default table output: `default security group rule list`, `local ip list`, `network flavor list`, `network flavor profile list`, `network meter list`, `network meter rule list`, `network segment range list`, `router ndp proxy list`, `tap service list`, `tap flow list`, and `tap mirror list`.

Remaining validation gaps: write/action lifecycle tests still need clouds exposing the relevant Neutron extensions and safe disposable fixtures. The current implementation should stay at `implemented` in the matrix until create, set/update, delete, and action paths get paired Python-vs-Go validation or documented extension-gated error parity.

Sources consulted:

* Local Python OSC command sources under `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/openstackclient/network/v2/`, especially `default_security_group_rule.py`, `local_ip.py`, `local_ip_association.py`, `network_agent.py`, `network_auto_allocated_topology.py`, `network_flavor.py`, `network_flavor_profile.py`, `l3_conntrack_helper.py`, `network_meter.py`, `network_meter_rule.py`, `network_segment_range.py`, `ndp_proxy.py`, and `taas/*.py`.
* Local OpenStackSDK resource sources under `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/openstack/network/v2/`, used to confirm resource keys and base paths.
* OpenStackClient default security group rule docs: https://static.opendev.org/docs/python-openstackclient/2024.2/cli/command-objects/default-security-group-rule.html.
* OpenStackSDK Local IP resource docs: https://docs.openstack.org/openstacksdk/latest/user/resources/network/v2/local_ip.html.
...
## 2026-05-08: Remaining Compute v2 Command Coverage

Decision: implement the remaining pinned `openstack.compute.v2` command paths in the core Compute dispatcher, using typed Gophercloud Nova packages where they preserve OSC behavior and narrow Nova REST shims where the local typed package coverage is absent, deprecated upstream, or does not expose the OSC action. The complete pinned Compute command list now has handlers rather than generated stubs.

Work done: added handlers, command-local flag parsing, command-list registration, and matrix coverage for `aggregate create`, `aggregate delete`, `aggregate set`, `aggregate unset`, `aggregate add host`, `aggregate remove host`, `aggregate cache image`, `compute agent create`, `compute agent delete`, `compute agent set`, `compute service delete`, `compute service set`, and `host set`. Aggregate CRUD and host membership use Gophercloud's aggregate package. Compute service updates use the Gophercloud services package where the service ID path is available, with a legacy raw Nova path retained for older microversions. The aggregate image-cache action, deprecated `os-agents` write commands, and deprecated `os-hosts` write command use raw authenticated Nova REST calls.

Compatibility fixes found during validation: aggregate show/create field rendering was corrected to match the Python oracle's field names and ordering for `availability_zone`, `created_at`, `deleted_at`, `hosts`, `id`, `is_deleted`, `name`, `properties`, `uuid`, and `updated_at`; empty aggregate availability zones render as `null`. `aggregate list --long` now keeps `availability_zone` out of the displayed properties map, matching the observed OSC output shape. One caveat remains: on `cloud6`, Python aggregate create returned `uuid: null` while the Go client, using the project's current Compute microversion default of `2.53`, received and displayed a UUID on create; aggregate show returned UUIDs in both clients. That needs a microversion-specific parity decision before marking aggregate create fully compatible.

Validation: mocked endpoint tests cover aggregate create, metadata update, image-cache request shape, aggregate delete, compute service lookup/update by host and binary, deprecated compute agent create/set/delete, and deprecated host set. `go test ./internal/cli ./tools/matrix`, `make report`, `make build`, and a command-list check for `openstack.compute.v2` all passed, with zero Compute commands marked `(Not Implemented Yet)`. A safe `cloud6` live smoke passed for test-created aggregate create, set, show, unset, and delete. Real control-plane mutations for compute service delete/set, compute agent create/delete/set, host set, aggregate host membership, and aggregate image cache were not live-run in this pass because they can affect Nova service state, hypervisor host state, agent records, or image cache behavior outside a disposable test resource.

Sources consulted:

* Local Python OSC Compute command sources: `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/openstackclient/compute/v2/aggregate.py`, `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/openstackclient/compute/v2/agent.py`, `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/openstackclient/compute/v2/service.py`, and `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/openstackclient/compute/v2/host.py`.
* Local OpenStackSDK Compute sources: `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/openstack/compute/v2/aggregate.py` and `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/openstack/compute/v2/_proxy.py`.
* Gophercloud Compute aggregate package docs: https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/compute/v2/aggregates.
* Gophercloud Compute service package docs: https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/compute/v2/services.

## 2026-05-09: Placement v1 Command Coverage

Decision: implement the pinned `openstack.placement.v1` command surface in the core dispatcher for now while keeping the matrix service scope marked as plugin-provided Placement. Typed Gophercloud packages are used for resource classes, resource providers, provider aggregates, provider traits, traits, and allocation candidates where they preserve the `osc-placement` behavior. Narrow authenticated Placement REST calls are used for consumer allocation commands, project/user resource usage, and inventory write payloads where the typed helper either does not exist in the local Gophercloud version or does not preserve the Python command's partial-payload shape.

Work done: added handlers, command-local flags, command-list registration, matrix status, and mocked request-shape tests for all previously unimplemented Placement paths: `resource class create/delete/set`, `resource provider create/delete/set`, `resource provider aggregate set`, `resource provider allocation delete/set/unset`, `resource provider inventory class set/delete/set`, `resource provider trait delete/set`, `resource usage show`, and `trait create/delete`. Existing Placement read output was tightened to the local `osc-placement` field names and ordering for resource classes, resource providers, inventories, aggregates, traits, allocations, usage, and allocation candidates.

Compatibility fixes found during validation: `resource provider allocation show` needed to target consumer allocations at `/allocations/{consumer_uuid}`, not provider allocations, to match the Python plugin. `resource usage show` needed to keep usage values as arbitrary JSON values because the `cloud6` Placement API returns an `unknown` usage bucket whose value is an object, not a plain integer. The default Placement microversion needed to match the Python plugin's `MAX_VERSION_NO_GAP` value, `1.29`, because defaulting Go to `1.39` added a `consumer_type` column that the Python oracle did not emit unless that microversion was explicitly selected. The Go command now emits the Python-compatible warning when `--consumer-type` is passed below microversion `1.38`. Inventory validation showed that `resource provider inventory class set` must run after an inventory class exists on the provider; allocation validation showed that requested amounts must respect the provider's `step_size` inventory constraint.

Validation: `go test ./internal/cli`, `go test ./tools/matrix`, `make build`, `make report`, and a Placement command-list check passed with zero `openstack.placement.v1` commands marked `(Not Implemented Yet)`. A safe `cloud6` smoke passed for test-owned resource class, trait, resource provider create/set/delete, inventory set/class-set/list/delete, trait assignment/delete, aggregate assignment, consumer allocation set/show/unset/delete, project/user usage show, and allocation candidate list, with cleanup registered for every created resource. A paired Python-vs-Go JSON parity check also passed on `cloud6` for `resource class show`, `trait show`, `resource provider show`, `resource provider inventory list/show`, `resource provider trait list`, `resource provider aggregate list`, `resource provider allocation show`, `resource provider usage show`, and `resource usage show` against the same disposable provider, allocation, trait, and class fixtures.

Remaining parity risks: grouped allocation-candidate query behavior, repeated `--member-of` query parity, exact microversion error text, generation-conflict error text, and broader paired Python-vs-Go write-output parity still need promotion tests before Placement can be called fully compatible under A-029.

Sources consulted:

* Local Python Placement plugin entry points: `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/osc_placement-4.8.0.dist-info/entry_points.txt`.
* Local Python Placement command sources under `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/osc_placement/resources/`, especially `allocation.py`, `aggregate.py`, `inventory.py`, `resource_class.py`, `resource_provider.py`, `trait.py`, and `usage.py`.
* Local Python Placement functional tests: `/Users/ken/.local/share/uv/tools/python-openstackclient/lib/python3.12/site-packages/osc_placement/tests/functional/test_inventory.py`.
* Gophercloud Placement allocation candidates package docs: https://pkg.go.dev/github.com/gophercloud/gophercloud/v2/openstack/placement/v1/allocationcandidates.
